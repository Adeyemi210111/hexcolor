...
let colorCode = document.getElementById("hexcode");


btn.addEventListener('click', getColor);
function getColor() {
    let randomNum = Math.floor(Math.random() * hex.length);
    let bgColor = (body.style.backgroundColor = hex[randomNum].hexCode);
    console.log(bgColor);
     
    if (hex[randomNum].hexCode == '#000000' ||  hex[randomNum].hexCode == '#010203') {
        colorName.innerHTML = `<span style='color:white;'>${hex[randomNum].name}</span>`;
        colorCode.innerHTML = `<span style='color:white;'>${hex[randomNum].hexCode}</span>`;

    } else {
         colorName.innerHTML = `${hex[randomNum].name}`;
         colorCode.innerHTML = `${hex[randomNum].hexCode}`;
    }
